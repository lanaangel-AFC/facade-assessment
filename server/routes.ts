...
export async function registerRoutes(
  ...
  app.post("/api/projects/:projectId/observations", async (req, res) => {
    ...
    // If no system is linked at create time, leave it blank — it will be
    // generated by the PATCH handler when the system is linked.
  ...
  app.patch("/api/observations/:id", async (req, res) => {
    ...
    // human-readable observation ID yet, generate one now using the same
    // logic as the create path. This covers the case where an observation
    // was saved first without a system, and the system is added later.
    ...
    if (incomingSystemId) {
      ...
      if (current && (!current.observationId || current.observationId === "")) {
        patch.observationId = await storage.getNextObservationId(current.projectId, incomingSystemId, id);
      }
    }
